...
var _ = Describe("lifecycle-container-poststart", func() {
	...
	It("One deployment, one pod with postStart spec", func() {
		...
		err = globalhelper.ValidateIfReportsAreValid(tsparams.CertsuiteContainerStartUpTcName, globalparameters.TestCasePassed, randomReportDir)
		Expect(err).ToNot(HaveOccurred())
	})
	...
	It("One daemonSet without postStart spec [negative]", func() {
		...
		By("Verify test case status in Claim report")
		err = globalhelper.ValidateIfReportsAreValid(tsparams.CertsuiteContainerStartUpTcName, globalparameters.TestCaseFailed, randomReportDir)
		Expect(err).ToNot(HaveOccurred())
	})
