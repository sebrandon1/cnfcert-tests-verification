...
var _ = Describe("lifecycle-pod-high-availability", Serial, func() {
	...
	It("One deployment, replicas are more than 1, podAntiAffinity is set", func() {
		...
		err = globalhelper.ValidateIfReportsAreValid(tsparams.CertsuitePodHighAvailabilityTcName, globalparameters.TestCasePassed, randomReportDir)
		Expect(err).ToNot(HaveOccurred())
	})
	...
	It("One deployment, replicas are more than 1, podAntiAffinity is not set [negative]", func() {
		...
		By("Verify test case status in Claim report")
		err = globalhelper.ValidateIfReportsAreValid(tsparams.CertsuitePodHighAvailabilityTcName, globalparameters.TestCaseFailed, randomReportDir)
		Expect(err).ToNot(HaveOccurred())
	})
