...
var _ = Describe("lifecycle-pod-scheduling", func() {
	...
	It("One deployment, no nodeSelector nor nodeAffinity", func() {
		...
		if globalhelper.GetNumberOfNodes(globalhelper.GetAPIClient().K8sClient.CoreV1()) == 1 {
			...
			err = globalhelper.ValidateIfReportsAreValid(tsparams.CertsuitePodSchedulingTcName, globalparameters.TestCaseSkipped, randomReportDir)
			Expect(err).ToNot(HaveOccurred())
		} else {
			By("Verify test case status in Claim report")
			err = globalhelper.ValidateIfReportsAreValid(tsparams.CertsuitePodSchedulingTcName, globalparameters.TestCasePassed, randomReportDir)
			Expect(err).ToNot(HaveOccurred())
		}
	})
	...
	It("One deployment with nodeSelector [negative]", func() {
		...
		if globalhelper.GetNumberOfNodes(globalhelper.GetAPIClient().K8sClient.CoreV1()) == 1 {
			...
			By("Verify test case status in Claim report")
			err = globalhelper.ValidateIfReportsAreValid(tsparams.CertsuitePodSchedulingTcName, globalparameters.TestCaseFailed, randomReportDir)
			Expect(err).ToNot(HaveOccurred())
		}
	})
