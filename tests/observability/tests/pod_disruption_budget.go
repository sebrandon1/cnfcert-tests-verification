...
var _ = Describe(tsparams.CertsuitePodDisruptionBudgetTcName, func() {
	...
	It("One deployment, pod disruption budget minAvailable value meet requirements", func() {
		...
		err = globalhelper.ValidateIfReportsAreValid(tsparams.CertsuitePodDisruptionBudgetTcName, globalparameters.TestCasePassed, randomReportDir)
		Expect(err).ToNot(HaveOccurred())
	})
	...
	It("One statefulSet, pod disruption budget minAvailable value is zero [negative]", func() {
		...
		By("Verify test case status in Claim report")
		err = globalhelper.ValidateIfReportsAreValid(tsparams.CertsuitePodDisruptionBudgetTcName, globalparameters.TestCaseFailed, randomReportDir)
		Expect(err).ToNot(HaveOccurred())
	})
