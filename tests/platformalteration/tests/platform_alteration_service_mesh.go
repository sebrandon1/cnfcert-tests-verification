...
var _ = Describe("platform-alteration-service-mesh-usage-installed", Ordered, func() {
	...
	It("istio is installed", func() {
		...
		err = globalhelper.ValidateIfReportsAreValid(tsparams.CertsuiteServiceMeshUsageName, globalparameters.TestCasePassed, randomReportDir)
		Expect(err).ToNot(HaveOccurred())
	})
	...
	It("istio is installed but proxy containers does not exist [negative]", func() {
		...
		err = globalhelper.ValidateIfReportsAreValid(tsparams.CertsuiteServiceMeshUsageName, globalparameters.TestCaseFailed, randomReportDir)
		Expect(err).ToNot(HaveOccurred())
	})
...
var _ = Describe("platform-alteration-service-mesh-usage-uninstalled", Serial, func() {
	...
	It("istio is not installed", func() {
		...
		err = globalhelper.ValidateIfReportsAreValid(tsparams.CertsuiteServiceMeshUsageName, globalparameters.TestCaseSkipped, randomReportDir)
		Expect(err).ToNot(HaveOccurred())
	})
})
